...
export default function Cart() {
    ...
    const handleSetQuantity = (event, index) => {
        const newInputValues = [...quantity]; // Create a copy of inputValues array
        newInputValues[index] = event.target.value; // Update the value for the corresponding item
        setQuantity(newInputValues); // Update the state
    };
    ...
    useEffect(() => {
        ...
        quantity.forEach((item, index) => {
            total += item * cartData[index].product.price;
        });
        setTotalPrice(total);
    }, [quantity]);
    // const quantityArray = [];
    ...
    return (
        <>
            {/* <div className="mx-auto w-full max-w-[1024px] mt-16 py-12">
                <h1 className="text-[40px] font-semibold tracking-tight">Review your bag</h1>
                <h2 className="pt-4">Free Delivery and free returns.</h2>
                ...
                <div class="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
                            ...
                            <div key={index}>
                                <div class="grid grid-cols-1 lg:grid-cols-2 min-[550px]:gap-6 border-t border-gray-200 py-6">
                                    <div class="flex items-center flex-col min-[550px]:flex-row gap-3 min-[550px]:gap-6 w-full max-xl:justify-center max-xl:max-w-xl max-xl:mx-auto">
                                        <div class="img-box">
                                            <img src={`https://picsum.photos/id/${index * 20}/200`} alt="perfume bottle image" class="xl:w-[140px]" />
                                        </div>
                                        <div class="pro-data w-full max-w-sm ">
                                            <h5 class="font-semibold text-xl leading-8 text-black max-[550px]:text-center">{item.product.title}</h5>
                                    ...
                                    <div class="flex items-center flex-col min-[550px]:flex-row w-full max-xl:max-w-xl max-xl:mx-auto gap-2">
                                        <h6 class="font-manrope font-bold text-2xl leading-9 text-black w-full max-w-[176px] text-center">
                                            FREE <span class="text-sm text-gray-300 ml-3 lg:hidden whitespace-nowrap">(Delivery Charge)</span>
                                        </h6>
                                        ...
                                        <h6 class="text-indigo-600 font-manrope font-bold text-2xl leading-9 w-full max-w-[176px] text-center">${item.product.price * quantity[index]}</h6>
                                    </div>
                                </div>
                            </div>
                        ))}
                    <div class="bg-gray-50 rounded-xl p-6 w-full mb-8 max-lg:max-w-xl max-lg:mx-auto">
                        <div class="flex items-center justify-between w-full mb-6">
                            ...
                            <h6 class="font-semibold text-xl leading-8 text-gray-900">${totalPrice}.00</h6>
                        </div>
                        ...
                        <div class="flex items-center justify-between w-full py-6">
                            ...
                            <h6 class="font-manrope font-medium text-2xl leading-9 text-indigo-500">${totalPrice}.00</h6>
                        </div>
                    </div>
